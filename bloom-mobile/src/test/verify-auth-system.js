...
const mockAuth = {
  async signUp(data) {
    try {
      ...
      zodiac_sign: 'Aries', // This would normally be calculated
...
async function verifyAuthSystem() {
  ...
  try {
    ...
    testUser = testData; // Save for sign in test
  ...
  try {
    ...
    if (!Array.isArray(profile.personality_traits)) {
      throw new Error('Personality traits not initialized as array');
    }
    if (!Array.isArray(profile.lifestyle_ratings)) {
      throw new Error('Lifestyle ratings not initialized as array');
    }
    if (!Array.isArray(profile.values_ratings)) {
      throw new Error('Values ratings not initialized as array');
    }
    ...
    const testUpdates = {
      ...
      lifestyle_ratings: ['Active', 'Social'],
      values_ratings: ['Family-oriented', 'Career-driven']
    };
    ...
    if (!refreshedProfile.personality_traits.includes('Adventurous') ||
        !refreshedProfile.lifestyle_ratings.includes('Active') ||
        !refreshedProfile.values_ratings.includes('Family-oriented')) {
