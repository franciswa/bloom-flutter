...
async function testMessaging() {
  try {
    ...
    const { data: editedMessage, error: editError } = await supabase
      .from('messages')
      .update({
        content: 'Edited message from user 1!',
    ...
    const typingChannel = supabase
      ...
      .on('broadcast', { event: 'typing' }, (payload) => {
        console.log('Received typing status:', payload.payload);
      })
      .subscribe();
    ...
    await typingChannel.send({
      type: 'broadcast',
      event: 'typing',
      payload: { userId: user1.user.id, isTyping: true }
    });
