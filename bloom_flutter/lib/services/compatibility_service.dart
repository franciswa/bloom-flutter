...
class CompatibilityService {
  ...
  Future<Compatibility?> getCompatibilityByUserIds(
  ...
  Future<Compatibility> calculateCompatibility(
      String userId1, String userId2) async {
    // Get charts
    ...
    // Calculate compatibility
    final compatibility =
        await calculateCompatibilityFromCharts(chart1, chart2);
  ...
  Future<Compatibility> calculateCompatibilityFromCharts(
    ...
    final questionnaireScore =
        await _calculateQuestionnaireScore(chart1.userId, chart2.userId);

    // Calculate overall score (weighted average)
    final overallScore =
        ((astrologicalScore * 0.5) + (questionnaireScore * 0.5)).round();

    // Calculate overall level
    final overallLevel =
        Compatibility.getCompatibilityLevelFromScore(overallScore);
    ...
    return Compatibility(
      ...
      overallDescription:
          _getOverallDescription(overallLevel, chart1.sunSign, chart2.sunSign),
  ...
  Future<int> _calculateAstrologicalScore(Chart chart1, Chart chart2) async {
    ...
    final sunSignScore =
        _calculateSunSignCompatibility(chart1.sunSign, chart2.sunSign);

    // Calculate moon sign compatibility (25%)
    final moonSignScore =
        _calculateMoonSignCompatibility(chart1.moonSign, chart2.moonSign);

    // Calculate ascendant compatibility (15%)
    final ascendantScore =
        chart1.ascendantSign != null && chart2.ascendantSign != null
            ? _calculateAscendantCompatibility(
                chart1.ascendantSign!, chart2.ascendantSign!)
            : 50;
  ...
  Future<int> _calculateQuestionnaireScore(
      String userId1, String userId2) async {
    try {
      // Get questionnaire result
      final questionnaireResult =
          await _questionnaireService.getQuestionnaireResult(userId1, userId2);
      ...
      final newResult = await _questionnaireService
          .calculateQuestionnaireCompatibility(userId1, userId2);
  ...
  int _calculateSunSignCompatibility(ZodiacSign sign1, ZodiacSign sign2) {
    ...
    final elementScore =
        _calculateElementCompatibilityScore(element1, element2);

    // Calculate modality compatibility (0-100)
    final modalityScore =
        _calculateModalityCompatibilityScore(modality1, modality2);
  ...
  Map<Element, int> _countElements(Chart chart) {
    ...
    elementCounts[sunElement] =
        elementCounts[sunElement]! + 3; // Sun has higher weight

    // Count moon sign element
    final moonElement = _chartService.getElementForSign(chart.moonSign);
    elementCounts[moonElement] =
        elementCounts[moonElement]! + 2; // Moon has higher weight

    // Count ascendant element
    if (chart.ascendantSign != null) {
      final ascendantElement =
          _chartService.getElementForSign(chart.ascendantSign!);
      elementCounts[ascendantElement] =
          elementCounts[ascendantElement]! + 2; // Ascendant has higher weight
    }
  ...
  int _calculateModalityCompatibilityScore(
  ...
  int _calculateSignCompatibilityScore(ZodiacSign sign1, ZodiacSign sign2) {
    ...
    int baseScore = (_calculateElementCompatibilityScore(element1, element2) *
                0.6 +
            _calculateModalityCompatibilityScore(modality1, modality2) * 0.4)
        .round();
  ...
  Future<List<CompatibilityCategory>> _calculateCategories(
      Chart chart1, Chart chart2) async {
    // Get questionnaire result
    QuestionnaireResult? questionnaireResult;
    try {
      questionnaireResult = await _questionnaireService.getQuestionnaireResult(
          chart1.userId, chart2.userId);
    } catch (e) {
      // Ignore errors
    }
    ...
    final emotionalScore =
        _calculateMoonSignCompatibility(chart1.moonSign, chart2.moonSign);
    final emotionalLevel =
        Compatibility.getCompatibilityLevelFromScore(emotionalScore);
    categories.add(CompatibilityCategory(
      name: 'Emotional',
      score: emotionalScore,
      level: emotionalLevel,
      description: _getEmotionalDescription(
          emotionalLevel, chart1.moonSign, chart2.moonSign),
    ));

    // Communication compatibility (based on Mercury signs)
    final communicationScore =
        chart1.mercurySign != null && chart2.mercurySign != null
            ? _calculateSunSignCompatibility(
                chart1.mercurySign!, chart2.mercurySign!)
            : 60;
    final communicationLevel =
        Compatibility.getCompatibilityLevelFromScore(communicationScore);
    categories.add(CompatibilityCategory(
      name: 'Communication',
      score: communicationScore,
      level: communicationLevel,
      description:
          _getCommunicationDescription(communicationLevel, chart1, chart2),
    ));

    // Physical compatibility (based on Mars and Venus signs)
    final physicalScore = chart1.marsSign != null &&
            chart2.venusSign != null &&
            chart1.venusSign != null &&
            chart2.marsSign != null
        ? (_calculateSunSignCompatibility(chart1.marsSign!, chart2.venusSign!) +
                _calculateSunSignCompatibility(
                    chart1.venusSign!, chart2.marsSign!)) ~/
            2
        : 65;
    final physicalLevel =
        Compatibility.getCompatibilityLevelFromScore(physicalScore);
    categories.add(CompatibilityCategory(
      name: 'Physical',
      score: physicalScore,
      level: physicalLevel,
    ...
    final intellectualScore =
        chart1.mercurySign != null && chart2.mercurySign != null
            ? (_calculateSunSignCompatibility(
                        chart1.mercurySign!, chart2.mercurySign!) +
                    _calculateSunSignCompatibility(
                        chart1.sunSign, chart2.sunSign)) ~/
                2
            : 60;
    final intellectualLevel =
        Compatibility.getCompatibilityLevelFromScore(intellectualScore);
    categories.add(CompatibilityCategory(
      name: 'Intellectual',
      score: intellectualScore,
      level: intellectualLevel,
      description:
          _getIntellectualDescription(intellectualLevel, chart1, chart2),
    ));

    // Spiritual compatibility (based on Jupiter and Neptune signs)
    final spiritualScore = chart1.jupiterSign != null &&
            chart2.jupiterSign != null &&
            chart1.neptuneSign != null &&
            chart2.neptuneSign != null
        ? (_calculateSunSignCompatibility(
                    chart1.jupiterSign!, chart2.jupiterSign!) +
                _calculateSunSignCompatibility(
                    chart1.neptuneSign!, chart2.neptuneSign!)) ~/
            2
        : 60;
    final spiritualLevel =
        Compatibility.getCompatibilityLevelFromScore(spiritualScore);
    categories.add(CompatibilityCategory(
      name: 'Spiritual',
      ...
      for (final category in QuestionnaireCategory.values) {
        ...
        switch (category) {
          case QuestionnaireCategory.personalValues:
            name = 'Values';
            description =
                'Your compatibility in terms of values and beliefs is ${Compatibility.getCompatibilityLevelName(level).toLowerCase()}.';
            break;
          case QuestionnaireCategory.lifestyle:
            name = 'Lifestyle';
            description =
                'Your compatibility in terms of lifestyle preferences is ${Compatibility.getCompatibilityLevelName(level).toLowerCase()}.';
            break;
          case QuestionnaireCategory.communication:
            name = 'Communication Style';
            description =
                'Your compatibility in terms of communication style is ${Compatibility.getCompatibilityLevelName(level).toLowerCase()}.';
            break;
          case QuestionnaireCategory.intimacy:
            name = 'Intimacy';
            description =
                'Your compatibility in terms of intimacy preferences is ${Compatibility.getCompatibilityLevelName(level).toLowerCase()}.';
            break;
          case QuestionnaireCategory.futureGoals:
            name = 'Future Goals';
            description =
                'Your compatibility in terms of future goals is ${Compatibility.getCompatibilityLevelName(level).toLowerCase()}.';
            break;
        }
  ...
  Future<List<CompatibilityAspect>> _calculateAspects(
      Chart chart1, Chart chart2) async {
    // Simplified aspects
    return [
      CompatibilityAspect(
        ...
        score: 90,
        description:
            _getAspectDescription(Aspect.conjunction, Planet.sun, Planet.sun),
      ),
      CompatibilityAspect(
        aspect: Aspect.trine,
        ...
        score: 85,
        description:
            _getAspectDescription(Aspect.trine, Planet.moon, Planet.moon),
      ),
    ];
  }
  ...
  Future<String> _generateCompatibilityReport(
    ...
    buffer.writeln();
    buffer.writeln(
        'Your overall compatibility score is $overallScore%, which is considered ${Compatibility.getCompatibilityLevelName(overallLevel).toLowerCase()}.');
    buffer.writeln();
    ...
    buffer.writeln();
    buffer.writeln(
        'Your astrological compatibility is based on the positions of planets in your birth charts:');
    buffer.writeln();
    ...
    if (chart1.ascendantSign != null && chart2.ascendantSign != null) {
      buffer.writeln(
          '- ${chart1.ascendantSign} Ascendant + ${chart2.ascendantSign} Ascendant');
    }
    buffer.writeln();
    ...
    if (questionnaireScore > 0) {
      buffer.writeln(
          'Your questionnaire compatibility is based on your answers to questions about values, lifestyle, communication, intimacy, and future goals.');
    } else {
      buffer.writeln(
          'You have not completed the compatibility questionnaire yet. Complete it to get a more accurate compatibility score.');
    }
    buffer.writeln();
    ...
    for (final category in categories) {
      ...
      buffer.writeln();
      buffer.writeln(
          'Score: ${category.score}% (${Compatibility.getCompatibilityLevelName(category.level)})');
      buffer.writeln();
    ...
    for (final aspect in aspects) {
      buffer.writeln(
          '- ${aspect.firstUserPlanet.toString().split('.').last} ${aspect.aspect.toString().split('.').last} ${aspect.secondUserPlanet.toString().split('.').last} (${aspect.score}%)');
  ...
  String _getOverallDescription(
      CompatibilityLevel level, ZodiacSign sign1, ZodiacSign sign2) {
    switch (level) {
  ...
  String _getEmotionalDescription(
      CompatibilityLevel level, ZodiacSign sign1, ZodiacSign sign2) {
    final element1 = _chartService.getElementForSign(sign1);
    final element2 = _chartService.getElementForSign(sign2);
    ...
    if (element1 == element2) {
      elementDescription =
          'Your emotional natures are similar, both being ${element1.toString().split('.').last} signs.';
    } else {
      elementDescription =
          'Your emotional natures are different, with ${sign1.toString().split('.').last} being a ${element1.toString().split('.').last} sign and ${sign2.toString().split('.').last} being a ${element2.toString().split('.').last} sign.';
    }
  ...
  String _getCommunicationDescription(
      CompatibilityLevel level, Chart chart1, Chart chart2) {
  ...
  String _getPhysicalDescription(
      CompatibilityLevel level, Chart chart1, Chart chart2) {
  ...
  String _getIntellectualDescription(
      CompatibilityLevel level, Chart chart1, Chart chart2) {
  ...
  String _getSpiritualDescription(
      CompatibilityLevel level, Chart chart1, Chart chart2) {
